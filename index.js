var __viboplrPlugin = function(exports) {
  ...
  const GROOVE_BASE = 0.125;
  const WAVE_PRESENCE = 0.075;
...
`;
  function createVinylDeckVisualizer(options2) {
    let host;
    let root;
    let deck;
    let platter;
    ...
    function repress(tracks) {
      ...
      const crop = buildCrop(options2.composition, geo);
    ...
    function renderGapRings() {
      ...
      if (!options2.emphasiseGaps) return;
    ...
    return {
      ...
      frame(state) {
        const applied = `${options2.composition}|${options2.emphasiseGaps}`;
